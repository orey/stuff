...
def footprint_sha1(filename):
    '''
    Modified to take in charge long pathnames
    '''
    longfilename = f"\\\\?\\{filename}"
    with open(longfilename, 'rb') as doc:
        content = doc.read()
...
def myprint(obj, end="\n" , flush=True, verbose=True):
    ...
    if FIRST:
        with open(LOG, "w",encoding='utf-8') as logfile:
            logfile.write(str(obj) + end)
        FIRST = False
    else:
        with open(LOG, "a",encoding='utf-8') as logfile:
            logfile.write(str(obj) + end)
    if verbose:
        print(str(obj), end=end, flush=flush)
...
class Timer():
    ...
    def stop(self):
        self.stop = time.time()
        minutes = (round(self.stop-self.start))//60
        seconds = (round(self.stop-self.start))%60
