...
'=== Useless because it cannot be passed in interfaces
...
Function ExportTheFolder(folder As Outlook.folder) As Integer
    ' Adding the subfolder stuff
    Dim subfolder As String
    subfolder = CleanString(folder.name)
    Dim theRealOutputFolder As String
    theRealOutputFolder = OUTPUTFOLDER & subfolder & "\"
    CreateDir (theRealOutputFolder)
    ...
    For Each item In folder.items
        If TypeName(item) = "MailItem" Then
            ...
            If Not IsFile(theRealOutputFolder & fullname) Then
                myitem.SaveAs theRealOutputFolder & fullname
                myitem.SaveAs theRealOutputFolder & textname, olTXT
            ...
            If atts.Count <> 0 Then
                For Each att In atts
                    ...
                    On Error Resume Next
                        ...
                        If Not IsFile(theRealOutputFolder & datedisplay & "-" & myatt.FileName) Then
                            myatt.SaveAsFile theRealOutputFolder & datedisplay & "-" & myatt.FileName
                            realcount = realcount + 1
                        Else
